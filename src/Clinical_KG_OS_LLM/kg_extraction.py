...
- For MEDICAL_HISTORY: extract only what a doctor would write in a clinical note for this visit — confirmed positive facts (smoking, COPD history), clinically relevant negations (non-smoker for a respiratory visit), medication adherence events (missed dose, stopped medication), and family history or exposure only when relevant to the current diagnosis or complaint. One node per concept using the most informative phrasing — do not fragment (e.g. "family history of lung cancer" not "father had lung cancer" as a separate node). Do not extract routine screening negations or individual vaccine names — use a single summary if immunization status matters (e.g. "up to date on vaccinations").
